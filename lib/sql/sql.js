...
exports.table = function (table) {
    var name = table.name||table.table;
    return (this.client.pg)
        ? ['"',table.schema||this.schema,'"."',name,'"'].join('')
        : ['`',name,'`'].join('');
}
...
exports.joins = function (table, fk, ref, pk, alias) {
    var result = [
        ' LEFT JOIN',this.table(ref),alias&&this.justName(alias),'ON',
        this.andColumns(table, fk, alias||ref.table, pk)
    ].join(' ');
    return result;
}
