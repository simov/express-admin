...
function createColumn (name, info) {
    var control;
    var braceIndex = info.type.indexOf('(');
    var typeName = braceIndex>-1 ? info.type.substr(0,braceIndex) : info.type;
    switch (typeName){
      case "integer":
        control={number: true};
        break;
      case "boolean":
        control={radio: true, options:["True","False"]};
        break;
      case "datetime":
        control={datetime:true};
        break;
      case "date":
        control={date:true};
        break;
      case "time":
        ...
        break;
      case "text":
        control={textarea:true, "editor": "html-editor"};
        break;
      default :
        if (info.key==='for'){
          control = {select:true};
