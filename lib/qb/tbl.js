...
exports.select = function (args) {
    ...
    for (var i=0; i < columns.length; i++) {
        if (columns[i].oneToMany && columns[i].fk) {
            names.push(z.as(table, columns[i].fk, columns[i].name,','));
        }
...
exports.insert = function (args, settings, record) {
    ...
    var str = query.replace('insert',
            x.name(settings.table.name,z.schema(settings.table)), 
            data.keys.join(),
            data.values.join(),
            sql.list('',settings.table.pk));
...
exports.update = function (args, settings, record) {
    ...
    var str = query.replace('update',
            x.name(settings.table.name,z.schema(settings.table)),
            data.pairs.join(),
            x.and(z.eq(settings.table, pk, record.pk)));
...
exports.remove = function (args, settings, record) {
    ...
    var str = query.replace('delete',
            x.name(settings.table.name,z.schema(settings.table)),
            x.and(z.eq(settings.table,
            pk,
            record.pk)));
