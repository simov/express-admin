...
exports.select = function (args) {
    ...
    for (var i=0; i < columns.length; i++) {
        ...
        if (columns[i].oneToMany || columns[i].manyToMany) {
            var result = this.join(table, columns[i], i);
    ...
    var where = this.statement(table, columns, args.filter, joins);
...
exports.statement = function (table, columns, filter, joins) {
    ...
    for (var i=0; i < columns.length; i++) {
        ...
        if (column.oneToMany && column.fk) {
            ...
            var expr = this.expression(table, column.name, column.control, value);
            if (!expr) continue;
