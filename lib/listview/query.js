...
exports = module.exports = function (args) {
    ...
    for (var i=0; i < columns.length; i++) {
        ...
        if (columns[i].oneToMany) {
            ...
            names.push(sql.group(ref.table, ref.columns, columns[i].name, alias));
            if (columns[i].fk) {
                joins.push(sql.joins(table.name, columns[i].fk, ref, ref.pk, alias));
            } else {
                joins.push(sql.joins(table.name, columns[i].name, ref, ref.pk, alias));
            }
        }
            ...
            joins.push(sql.joins(table.name, table.pk, link, link.parentPk));
            joins.push(sql.joins(link.table, link.childPk, ref, ref.pk));
        }
        else {
