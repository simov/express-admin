        ...
        if (record.insert) {
            var data = this._kvp(args.name, record.columns, settings.columns),
                str = query.replace('insert',
                    sql.table(settings.table), data.keys.join(), data.values.join(), sql.list('',settings.table.pk));
            
            var action = 'insert';

        } else if (record.remove || args.action == 'remove') {
            var pk = settings.table.pk;
                str = query.replace('delete', sql.table(settings.table), sql.andValues(settings.table.name, pk, record.pk));
            ...
            var pk = settings.table.pk,
                ...
                str = query.replace('update',
                    sql.table(settings.table), data.pairs.join(), sql.andValues(settings.table.name, pk, record.pk));
