...
var tbl = {
    _getSql: function (args) {
        ...
        return query.replace('get-record',
            names.join(), table.name, sql.andValues(table.name, (args.fk? args.fk:table.pk), args.id));
    },
    // public - add docs
    get: function (args, cb) {
        if (args.post) {
...
var mtm = {
    ...
    _getIds: function (column, pk, args, cb) {
        ...
        var str = query.replace('get-record',
            sql.concat(link.table, link.childPk), link.table, sql.andValues(link.table, link.parentPk, pk));
