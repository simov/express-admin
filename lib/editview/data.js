...
var tbl = {
    _getSql: function (args) {
        ...
        return query.replace('get-record',
            names.join(), sql.table(table), sql.andValues(table, (args.fk? args.fk:table.pk), args.id));
    },
    // public - add docs
    get: function (args, cb) {
        if (args.post) {
...
var mtm = {
    ...
    _getIds: function (column, pk, args, cb) {
        ...
        var str = query.replace('get-record',
            sql.concat(link, link.childPk), sql.table(link), sql.andValues(link, link.parentPk, pk));
